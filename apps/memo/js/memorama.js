var directorio = "img/sc2personajes" //directorio o ruta a donde se guardan las imágenes
var numImagenes = 14 //cantidad de imágenes que existan em el directorio
perder = 20 //Numero de intentos a los que se pierde

//***No tocar el código a partir de aquí
var nums = new Array()    //Arreglo nums sin dimension
var cant = 8    //cantidad de pares en el juego
var acrtos = 0  //numero de aciertos
var intentos = 0    //numeros de intentos
gana = false    //boleano de si gana
perdio = false      //boletano de si pierde
...
document.writeln('<article class="menu">');
    document.writeln('<section class="title">');
        document.writeln('<p>MemoStarcraft</p>');
    document.writeln('</section>');
    document.writeln('<section id="account" class="account">');
        document.writeln('<div id="tiempo" class="tiempo">');
            document.writeln('Tiempo');
        document.writeln('</div>');
        document.writeln('<div id="datos" class="datos">');
        document.writeln('<a href="#" onclick="login()">Entrar!</a>');
        document.writeln('</div>');
    document.writeln('</section>');
...
document.writeln('<div id="contenedorMemorama">');
document.writeln('<table align="center" style="border: 1px solid red;">')
...
for (p = 1; p <= nums.length; p++) {
    ...
    document.write('<img id="' + lista[p - 1] + '" onclick="gira(this,this.id)" src="' + directorio + '/logo.jpg" width="80" height="80">')
...
function restaura() {
    gi1.src = directorio + "/" + "logo.jpg";
    gi1 = ""
    setTimeout('gi2.src = directorio + "/logo.jpg";gi2=""', 200)
    cont = 0
    intentos++
    document.formu.visor.value = "Intentos: " + intentos
    if (intentos >= perder) {
...
function finJuego(cual) {
    ...
    if (cual == 'pierde') {
        document.getElementById('mensaje').innerHTML = 'Agotaste tus ' + perder + ' intentos<br> Perdiste  :-(';
        cont = 'perdio'
    }
