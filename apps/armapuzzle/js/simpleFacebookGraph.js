...
function crearTabla() {
    ...
    var imagen = 1;
    var word = ['a','b','c','d'];
    var tabla = '<table class="tmemo">';
    for (var i = 0; i < 4; i++) {
        tabla += '<tr>';
        for (var j = 0; j < 4; j++) {
            //tabla += '<td>'+ word[i]+ '_'+ j + '<td/>';
            tabla += '<td><input type="button" onclick="obtenerValor(this)"  id="'+word[i]+'_'+ j +'" value="'+word[i]+'_'+ j +'" /><img src="'+ ruta + prefijo + imagen + extension +'"></td>\n';
            imagen++;
            if (imagen == 5) {
                imagen = 1;
            }
    ...
    tabla += '</table>';
    document.getElementById('memorama').innerHTML = tabla;
}

function obtenerValor(boton) {

    jQuery("input[id='test']").click(function(){
